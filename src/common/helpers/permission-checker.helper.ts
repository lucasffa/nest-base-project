...
export class PermissionChecker {
  ...
  canPerformAction(requesterUuid: string, targetUuid: string, requesterRole: Role, ...requiredPermissions: Permission[]): boolean {
    ...
    const canPerformOnOwn = this.hasPermission(requesterRole, Permission.READ_OWN_USER, Permission.UPDATE_OWN_USER, Permission.TOGGLE_OWN_USER_STATUS, Permission.DELETE_OWN_USER) && requesterUuid === targetUuid;
